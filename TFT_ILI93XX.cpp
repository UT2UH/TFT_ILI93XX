...
**********************************************************/
	...
	TFT_ILI93XX::TFT_ILI93XX(const uint8_t cspin,const uint8_t dcpin,const uint8_t rstpin,const uint8_t mosi,const uint8_t sclk)
	{
		_cs   = cspin;
		_dc   = dcpin;
		_rst  = rstpin;
	...
	TFT_ILI93XX::TFT_ILI93XX(const uint8_t cspin,const uint8_t dcpin,const uint8_t rstpin)
	{
		_cs   = cspin;
		_dc   = dcpin;
		_rst  = rstpin;
...
************************ backlight ***********************
**********************************************************/
void TFT_ILI93XX::useBacklight(const uint8_t pin)
{
	_bklPin = pin;
...
void TFT_ILI93XX::backlight(bool state)
{
	#if defined(__MK20DX128__) || defined(__MK20DX256__) || defined(__MK64FX512__) || defined(__MK66FX1M0__)
		if (_bklPin != 255) {
...
//-----------------Arduino Uno, Leonardo, Mega, Teensy 2.0, any 8 bit AVR
	...
	void TFT_ILI93XX::setBitrate(uint32_t n)
	{
			if (n >= 8000000) {
...
void TFT_ILI93XX::_sendInitData(const uint8_t cmd,const uint8_t data[])
{
	uint8_t i;
	if (data[0] > 0){
...
*/
uint8_t TFT_ILI93XX::getErrorCode(void) 
{
	return _initError;
}

void TFT_ILI93XX::clearMemory(void)
{
	startTransaction();
	setAddrWindow_cont(
				0,
				0,
				TFT_ILI93XX_TFTWIDTH,
				TFT_ILI93XX_TFTHEIGHT
	);
	_pushColors_cont(_defaultBgColor,TFT_ILI93XX_CGRAM);
	#if defined(__MK20DX128__) || defined(__MK20DX256__) || defined(__MK64FX512__) || defined(__MK66FX1M0__)
...
*/
void TFT_ILI93XX::changeMode(const enum ILI93XX_modes m) 
{
	if (m != _currentMode){
...
uint8_t TFT_ILI93XX::getMode(void) 
{
	return _currentMode;
}

void TFT_ILI93XX::setArea(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1) 
{
	startTransaction();
...
void TFT_ILI93XX::setPartialArea(uint16_t top,uint16_t bott) 
{
	startTransaction();
...
uint8_t TFT_ILI93XX::getRotation(void)  
{
	return _rotation;
	//return TFT_ILI93XX_ROTCOMP[_rotation];//compensate rotation for display upsdide down, etc.
}

void TFT_ILI93XX::setRotation(uint8_t m) 
{
	_rotation = m % 4; // can't be higher than 3
	//_Mactrl_Data &= ~(0xF8);//clear bit 3..7,  F0 -> clear bit 4...7
	//_rotation = TFT_ILI93XX_ROTCOMP[_rotation];//compensate rotation for display upsdide down, etc.
	uint8_t MactrlData = 0;
	if (_rotation == 0){
		_portrait = false;
		MactrlData = MADCTL_MX | TFT_ILI93XX_CSPACE;
		_width  = TFT_ILI93XX_TFTWIDTH;
		_height = TFT_ILI93XX_TFTHEIGHT;
		defineScrollArea(_scrollTop,_scrollBottom);
	} else if (_rotation == 1){
		//bitSet(_Mactrl_Data,6); 
		//bitSet(_Mactrl_Data,5);
		_width  = TFT_ILI93XX_TFTHEIGHT;
		_height = TFT_ILI93XX_TFTWIDTH;
		MactrlData = MADCTL_MV | TFT_ILI93XX_CSPACE;
		_portrait = true;
	} else if (_rotation == 2){
		...
		_width  = TFT_ILI93XX_TFTWIDTH;
		_height = TFT_ILI93XX_TFTHEIGHT;
		MactrlData = MADCTL_MY | TFT_ILI93XX_CSPACE;
		_portrait = false;
		defineScrollArea(_scrollTop,_scrollBottom);
	} else {
		//bitSet(_Mactrl_Data,7); 
		...
		_width  = TFT_ILI93XX_TFTHEIGHT;
		_height = TFT_ILI93XX_TFTWIDTH;
		MactrlData = MADCTL_MX | MADCTL_MY | MADCTL_MV | TFT_ILI93XX_CSPACE;
		_portrait = true;
	}
	//if (TFT_ILI93XX_CSPACE > 0) bitSet(_Mactrl_Data,3);
	startTransaction();
	 writecommand_cont(CMD_MADCTL);
	 //writedata8_last(_Mactrl_Data);
	 writedata8_last(MactrlData);
	endTransaction();
...
/*
//00..FF
void TFT_ILI93XX::setBrightness(uint8_t val) 
{
	startTransaction();
	writecommand_cont(CMD_WDBRIGHT);
	writedata8_last(val);
	endTransaction();
}

void TFT_ILI93XX::test() 
{
	startTransaction();
	writecommand_cont(0x53);
	writedata8_last(0b00101100);
	endTransaction();
}
*/

int16_t TFT_ILI93XX::width(void) const {
	return _width;
}
 
int16_t TFT_ILI93XX::height(void) const {
	return _height;
}

int16_t TFT_ILI93XX::cgWidth(void) const {
	return TFT_ILI93XX_TFTWIDTH;
}
 
int16_t TFT_ILI93XX::cgHeight(void) const {
	return TFT_ILI93XX_TFTHEIGHT;
}
...
int16_t TFT_ILI93XX::getScrollTop(void)
{
	return _scrollTop;
}

int16_t TFT_ILI93XX::getScrollBottom(void)
{
...
*/
void TFT_ILI93XX::defineScrollArea(int16_t tfa, int16_t bfa)
{
	if (_rotation == 1 || _rotation == 3 || _rotation == 2) return;//no scroll for rot 1,3!
	uint16_t area = 0;
		if (tfa == 0 && bfa == 0) {
			bfa = 0;//special
			//tfa = TFT_ILI93XX_OFST[_rotation][1];
			...
			_scrollBottom = bfa;
			//tfa += TFT_ILI93XX_OFST[_rotation][1];
			bfa = _height - bfa/* - TFT_ILI93XX_OFST[_rotation][1]*/;
			area = _height - tfa - bfa;
		}
...
uint8_t TFT_ILI93XX::getScrollDirection(void) 
{
	return _scrollDir;
}

void TFT_ILI93XX::setScrollDirection(uint8_t dir) 
{
	_scrollDir = dir % 2;
}
...
*/
/**************************************************************************/
uint16_t TFT_ILI93XX::gradient(uint8_t val)
{
	uint8_t r = 0;
	uint8_t g = 0;
	uint8_t b = 0;
...
*/
/**************************************************************************/
uint16_t TFT_ILI93XX::colorInterpolation(uint16_t color1,uint16_t color2,uint16_t pos,uint16_t div)
{
...
*/
/**************************************************************************/
uint16_t TFT_ILI93XX::colorInterpolation(uint8_t r1,uint8_t g1,uint8_t b1,uint8_t r2,uint8_t g2,uint8_t b2,uint16_t pos,uint16_t div)
{
...
void TFT_ILI93XX::setBackground(uint16_t color) 
{
	_defaultBgColor = color;
}


void TFT_ILI93XX::setForeground(uint16_t color) 
{
	_defaultFgColor = color;
}

uint16_t TFT_ILI93XX::getBackground(void) 
{
	return _defaultBgColor;
}

uint16_t TFT_ILI93XX::getForeground(void) 
{
...
void TFT_ILI93XX::drawPixel(int16_t x, int16_t y, uint16_t color) 
{
...
void TFT_ILI93XX::fillScreen(uint16_t color) 
{
	startTransaction();
	setAddrWindow_cont(
				0,
	...
	_pushColors_cont(color,TFT_ILI93XX_CGRAM);
	#if defined(__MK20DX128__) || defined(__MK20DX256__) || defined(__MK64FX512__) || defined(__MK66FX1M0__)
		writecommand_last(CMD_NOP);
	#else
		disableCS();
...
//with gradient
void TFT_ILI93XX::fillScreen(uint16_t color1,uint16_t color2) 
{
	startTransaction();
	if (color1 != color2){
		...
		_pushColors_cont(color1,TFT_ILI93XX_CGRAM);
	}
	#if defined(__MK20DX128__) || defined(__MK20DX256__) || defined(__MK64FX512__) || defined(__MK66FX1M0__)
		writecommand_last(CMD_NOP);
	#else
...
void TFT_ILI93XX::clearScreen(void) 
{
	fillScreen(_defaultBgColor);
	_cursorX = _cursorY = 0;
}
...
*/
void TFT_ILI93XX::drawFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color) 
{
...
then set CS hi and close transaction
*/
void TFT_ILI93XX::drawFastHLine(int16_t x, int16_t y, int16_t w, uint16_t color) 
{
	// Rudimentary clipping
	if (boundaryCheck(x,y)) return;
...
fill RECT
*/
void TFT_ILI93XX::fillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color) 
{
	if (boundaryCheck(x,y)) return;
	//if (((x + w) - 1) >= _width)  w = _width  - x;
	//if (((y + h) - 1) >= _height) h = _height - y;
...
*/
void TFT_ILI93XX::fillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color1,uint16_t color2) 
{
	if (boundaryCheck(x,y)) return;
	//if (((x + w) - 1) >= _width)  w = _width  - x;
	//if (((y + h) - 1) >= _height) h = _height - y;
...
void TFT_ILI93XX::fillRect_cont(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color1,uint16_t color2) 
{
	//uint16_t colorTemp;
...
draw LINE
*/
void TFT_ILI93XX::drawLine(int16_t x0, int16_t y0,int16_t x1, int16_t y1, uint16_t color)
{
	startTransaction();
...
void TFT_ILI93XX::drawLine_cont(int16_t x0, int16_t y0,int16_t x1, int16_t y1, uint16_t color)
{
	if (y0 == y1) {
		if (x1 > x0) {
...
draw rect
*/
void TFT_ILI93XX::drawRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
	startTransaction();
		drawFastHLine_cont(x, y, w, color);
...
void TFT_ILI93XX::drawArcHelper(uint16_t cx, uint16_t cy, uint16_t radius, uint16_t thickness, float start, float end, uint16_t color) 
{
	int16_t xmin = 65535;
	int16_t xmax = -32767;
	int16_t ymin = 32767;
...
void TFT_ILI93XX::setArcParams(float arcAngleMax, int arcAngleOffset)
{
...
*/
/**************************************************************************/
float TFT_ILI93XX::cosDeg_helper(float angle)
{
...
float TFT_ILI93XX::sinDeg_helper(float angle)
{
	float rads = angle / (float)360 * 2 * PI;
	return sin(rads);
}
...
//fast
void TFT_ILI93XX::drawEllipse(int16_t cx,int16_t cy,int16_t radiusW,int16_t radiusH,uint16_t color)
{
	int16_t x,y;
...
//fast
void TFT_ILI93XX::plot4points_cont(uint16_t cx, uint16_t cy, uint16_t x, uint16_t y, uint16_t color)
{
...
//fast
void TFT_ILI93XX::drawCircle_cont(int16_t x,int16_t y,int16_t radius,uint8_t cornername,uint16_t color) 
{
	int16_t f			= 1 - radius;
	int16_t ddF_x		= 1;
...
//fast
void TFT_ILI93XX::fillCircle_cont(int16_t x, int16_t y, int16_t radius, uint8_t cornername, int16_t delta, uint16_t color) 
{
	int16_t f     		= 1 - radius;
	int16_t ddF_x 		= 1;
	int16_t ddF_y 		= -2 * radius;
...
//fast
void TFT_ILI93XX::drawCircle(int16_t x, int16_t y, int16_t radius, uint16_t color)
{
	startTransaction();
...
void TFT_ILI93XX::fillCircle(int16_t x, int16_t y, int16_t radius,uint16_t color) 
{
...
//fast
void TFT_ILI93XX::drawRoundRect(int16_t x, int16_t y, int16_t w,int16_t h, int16_t radius, uint16_t color) 
{
...
//fast
void TFT_ILI93XX::fillRoundRect(int16_t x, int16_t y, int16_t w,int16_t h, int16_t radius, uint16_t color) 
{
	if (x+w >= _width || y+h > _height) return;
	if (radius < 2) {
...
void TFT_ILI93XX::drawQuad(int16_t x0, int16_t y0,int16_t x1, int16_t y1,int16_t x2, int16_t y2,int16_t x3, int16_t y3, uint16_t color) 
{
	startTransaction();//open SPI comm
...
void TFT_ILI93XX::fillQuad(int16_t x0, int16_t y0,int16_t x1, int16_t y1,int16_t x2, int16_t y2, int16_t x3, int16_t y3, uint16_t color,bool triangled) 
{
	startTransaction();//open SPI comm
...
void TFT_ILI93XX::drawPolygon(int16_t x, int16_t y, uint8_t sides, int16_t diameter, float rot, uint16_t color)
{ 
...
void TFT_ILI93XX::drawMesh(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
	if (boundaryCheck(x,y)) return;
	//if (((x + w) - 1) >= _width)  w = _width  - x;
	//if (((y + h) - 1) >= _height) h = _height - y;
...
void TFT_ILI93XX::drawTriangle(int16_t x0, int16_t y0,int16_t x1, int16_t y1,int16_t x2, int16_t y2, uint16_t color) 
{
	startTransaction();
	drawLine_cont(x0, y0, x1, y1, color);
...
//85% fast
void TFT_ILI93XX::fillTriangle(int16_t x0, int16_t y0,int16_t x1, int16_t y1,int16_t x2, int16_t y2, uint16_t color) 
{
	startTransaction();
	fillTriangle_cont(x0,y0,x1,y1,x2,y2,color);//
	#if defined(__MK20DX128__) || defined(__MK20DX256__) || defined(__MK64FX512__) || defined(__MK66FX1M0__)
...
void TFT_ILI93XX::fillTriangle_cont(int16_t x0, int16_t y0,int16_t x1, int16_t y1,int16_t x2, int16_t y2, uint16_t color) 
{
	int16_t a, b, y, last;
...
*/
/**************************************************************************/
void TFT_ILI93XX::drawLineAngle(int16_t x, int16_t y, int angle, uint8_t length, uint16_t color,int offset)
{
	if (length < 2) {//NEW
		drawPixel(x,y,color);
	} else {
...
*/
/**************************************************************************/
void TFT_ILI93XX::drawLineAngle(int16_t x, int16_t y, int angle, uint8_t start, uint8_t length, uint16_t color,int offset)
{
...
*/
/**************************************************************************/
void TFT_ILI93XX::ringMeter(int val, int minV, int maxV, uint8_t x, uint8_t y, uint8_t r, uint16_t colorScheme,uint16_t backSegColor,int angle,uint8_t inc)
{
	if (inc < 5) inc = 5;
	if (inc > 20) inc = 20;
	if (r < 50) r = 20;
...
//fast
void TFT_ILI93XX::startPushData(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1) 
{
	startTransaction();
	setAddrWindow_cont(x0,y0,x1,y1);
}
...
//fast
void TFT_ILI93XX::pushData(uint16_t color) 
{
	writedata16_cont(color);
}

//fast
void TFT_ILI93XX::endPushData() 
{
	#if defined(__MK20DX128__) || defined(__MK20DX256__) || defined(__MK64FX512__) || defined(__MK66FX1M0__)
		writecommand_last(CMD_NOP);
	#else
...
//fast
void TFT_ILI93XX::pushColor(uint16_t color) 
{
	startTransaction();
...
void TFT_ILI93XX::drawIcon(int16_t x, int16_t y,const tIcon *icon,uint8_t scale,uint16_t f,uint16_t b,bool inverse)
{
...
void TFT_ILI93XX::drawImage(int16_t x, int16_t y,const tPicture *img,const enum ILI93XX_iconMods m,uint16_t b)
{
	uint16_t px = 0;
	uint16_t color;
	uint8_t currentY = 0;
...
void TFT_ILI93XX::drawBitmap(int16_t x, int16_t y,const uint8_t *bitmap, int16_t w, int16_t h,uint16_t color) 
{
	int16_t i, j;
...
void TFT_ILI93XX::drawBitmap(int16_t x, int16_t y,const uint8_t *bitmap, int16_t w, int16_t h,uint16_t color, uint16_t bg) 
{
	int16_t i, j;
	int16_t byteWidth = (w + 7) / 8;
	for (j = 0; j < h; j++) {
...
********************  Text Functions *********************
**********************************************************/
void TFT_ILI93XX::setCursor(int16_t x, int16_t y,enum ILI93XX_centerMode c) 
{
...
void TFT_ILI93XX::getCursor(int16_t &x, int16_t &y) 
{
	x = _cursorX;
	y = _cursorY;
}
...
void TFT_ILI93XX::setTextScale(uint8_t s) 
{
	_textScaleX = _textScaleY = (s > 0) ? s : 1;
}


void TFT_ILI93XX::setTextScale(uint8_t sx,uint8_t sy) 
{
	_textScaleX = (sx > 0) ? sx : 1;
	_textScaleY = (sy > 0) ? sy : 1;
}

void TFT_ILI93XX::setTextColor(uint16_t color) 
{
	_textForeground = _textBackground = color;
}

void TFT_ILI93XX::setTextColor(uint16_t frgrnd, uint16_t bckgnd) 
{
...
void TFT_ILI93XX::setTextWrap(boolean w) 
{
	_textWrap = w;
}

void TFT_ILI93XX::setCharSpacing(uint8_t space)
{
	_charSpacing = space;
}

void TFT_ILI93XX::setFontInterline(uint8_t distance)
{
	_fontInterline = distance;
}

void TFT_ILI93XX::setInternalFont(void)
{
	#if defined(_ILI93XX_DEF_FONT_PATH)
		setFont(&_ILI93XX_DEF_FONT_NAME);
	#else
...
int TFT_ILI93XX::_getCharCode(uint8_t ch)
{
	int i;
	//remap ?
...
*/
int TFT_ILI93XX::_STRlen_helper(const char* buffer,int len)
{
	int charIndex = -1;
	int i;
...
void TFT_ILI93XX::setFont(const tFont *font) 
{
	_currentFont = font;
...
Handle strings
*/
void TFT_ILI93XX::_textWrite(const char* buffer, uint16_t len)
{
	uint16_t i;
...
*/
bool TFT_ILI93XX::_renderSingleChar(const char c)
{
...
*/
void TFT_ILI93XX::_glyphRender_unc(
...
*/
void TFT_ILI93XX::_charLineRender(
									bool 			lineBuffer[],
									int 			charW,
									int16_t 		x,
									int16_t 		y,
...
*/
#if defined(__AVR__)
	void TFT_ILI93XX::_pushColors_cont(uint16_t data,uint32_t times){
		uint8_t i;
		enableDataStream();
		while(times--) {
			for (i=0;i<2;i++){
...
/* ========================================================================
   ...
   ========================================================================*/
	#if !defined(__MK20DX128__) && !defined(__MK20DX256__) && !defined(__MK64FX512__) && !defined(__MK66FX1M0__)
		void TFT_ILI93XX::writecommand_cont(const uint8_t c)
		{
			enableCommandStream();
			spiwrite(c);
		}

		void TFT_ILI93XX::writedata8_cont(uint8_t c)
		{
			enableDataStream();
			spiwrite(c);
		}

		void TFT_ILI93XX::writedata16_cont(uint16_t d)
		{
			enableDataStream();
			spiwrite16(d);
		}

		void TFT_ILI93XX::writecommand_last(const uint8_t c)
		{
			enableCommandStream();
			spiwrite(c);
			disableCS();
		...
		void TFT_ILI93XX::writedata8_last(uint8_t c)
		{
			enableDataStream();
			spiwrite(c);
			disableCS();
		...
		void TFT_ILI93XX::writedata16_last(uint16_t d)
		{
			enableDataStream();
			spiwrite16(d);
			disableCS();
...
/* ========================================================================
   ...
   ========================================================================*/
	...
	void TFT_ILI93XX::drawFastHLine_cont(int16_t x, int16_t y, int16_t w, uint16_t color)
	{
		setAddrWindow_cont(x, y, x + w - 1, y);
		do { writedata16_cont(color); } while (--w > 0);
	}

	void TFT_ILI93XX::drawFastVLine_cont(int16_t x, int16_t y, int16_t h, uint16_t color)
	{
		setAddrWindow_cont(x, y, x, y + h - 1);
		do { writedata16_cont(color); } while (--h > 0);
	}

	void TFT_ILI93XX::drawPixel_cont(int16_t x, int16_t y, uint16_t color)
	{
	...
	void TFT_ILI93XX::setAddrWindow_cont(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1)
	{
		#if defined(TFT_ILI93XX_OFSTNEED)
			x0 += TFT_ILI93XX_OFST[_rotation][0];
			x1 += TFT_ILI93XX_OFST[_rotation][0];
			y0 += TFT_ILI93XX_OFST[_rotation][1];
			y1 += TFT_ILI93XX_OFST[_rotation][1];
		#endif
	...
	bool TFT_ILI93XX::boundaryCheck(int16_t x,int16_t y)
	{
		if ((x >= _width) || (y >= _height)) return true;
		return false;
	}

	int16_t TFT_ILI93XX::sizeCheck(int16_t origin,int16_t len,int16_t maxVal)
	{
...
/*
void TFT_ILI93XX::printPacket(word data,uint8_t count){
